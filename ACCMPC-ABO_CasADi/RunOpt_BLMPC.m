function optSol = RunOpt_BLMPC(OPTsettings)
% PrecVehMPC Runs the baseline MPC over the simulation
%
%   Inputs:
...
% Get matrices
[H,c,G,~,~,g_lb,g_ub] = CreateQP_BL(OPTsettings,n_x,n_u,s_0,v_0,s_est,v_est,s_tv_est,t_0,a_minus1);
...
% HPIPM
    ...
    ng_num = (size(G,1)-N_hor*n_x - n_x - 2)/N_hor;
    optsHPIPM = struct;
    optsHPIPM.N = N_hor;
    optsHPIPM.nx = n_x*ones(N_hor+1,1);
    optsHPIPM.nu = [n_u*ones(N_hor,1);0];
    optsHPIPM.ng = [ng_num+n_x;ng_num*ones(N_hor-1,1);n_x-1];
...
for kk = 0:N_sim 
    ...
    [s_est, v_est] = EstimateVehicleTrajectory(OPTsettings,3,s_measured,v_measured,a_minus1,s_opt_prev_sol,v_opt_prev_sol);
    ...
    [H,c,G,z_lb,z_ub,g_lb,g_ub] = CreateQP_BL(OPTsettings,n_x,n_u,s_measured,v_measured,s_est,v_est,s_tv_est,t_0,a_minus1);
    ...
    if OPTsettings.createGifs
        ...
        if kk == 0
            [EVtrajPlot,EVpredPlot,TVtrajPlot,TVpredPlot] = InitiateAnimation_MPC(k,s_opt,v_opt,s_opt_prev_sol,v_opt_prev_sol,s_tv_est,v_tv_est,OPTsettings,'BLMPC');
        else
