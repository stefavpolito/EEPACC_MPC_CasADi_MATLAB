...
tau_est = zeros(length(ABMPCsol.a_opt),1);
TW_opt = tau_est;
FC_EFF = tau_est;
FC_tot_EFF = tau_est;

for i=2:length(ABMPCsol.a_opt)
    
    TW_opt(i,1) = max( 0 , (V.lambda*V.m*ABMPCsol.a_opt(i,1) + ...
                            V.F0 + ...
                            V.F2*ABMPCsol.v_opt(i,1)*ABMPCsol.v_opt(i,1)) * V.R_w );
    % UNITS: FC[g/s] = Poly11 ( V[m/s] , TW[Nm] )
    FC_EFF(i,1) = max( 0.25 , V.p00 + ...
                              V.p10 * ABMPCsol.v_opt(i,1) + ...
                              V.p01 * TW_opt(i,1)); % [g/s]
    FC_tot_EFF(i,1) = FC_tot_EFF(i-1,1) + FC_EFF(i,1)/1000*0.5; % [kg]

end

FE_EFF = max(FC_tot_EFF/0.835)/max(ABMPCsol.s_opt/1000)*100 % [L/100km]
FE_ref = TO_Urban.FE(end,1)
