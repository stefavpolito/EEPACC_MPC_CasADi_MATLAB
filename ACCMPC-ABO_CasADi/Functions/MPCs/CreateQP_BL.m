function [H,c,G,z_lb,z_ub,g_lb,g_ub] = CreateQP_BL(OPTsettings,n_x,n_u,s_0,v_0,s_est,v_est,s_tv_est,t_0,a_minus1)
% CreateQP_BL Creates the baseline QP for qpOASES and HPIPM, i.e. it builds the matrices and vectors that define the optimization problem
%
%   Inputs:
...
[~,v_lim_max,v_stop_max,v_TL_max,v_curv_max,a_min_est,a_max_est,j_min_est,j_max_est,~] = EstimateRouteAndComfortBounds(OPTsettings, 1, s_est, v_est, t_0, N);
...
for kk = 0:N-1
   ...
   %% === Constraints ===
    ...
    % qpOASES
        ...
        % Minimum and maximum bounds on states and controls
            % acceleration is limited by other acceleration constraint
    ...
    G(cstrInd,o+[scurr,xi_f])= [1,-1];
    g_lb(cstrInd) = -inf;
    g_ub(cstrInd) = s_tv_est(k) - h_min;
        cstrInd = cstrInd + 1;
    G(cstrInd,o+[scurr,vcurr,xi_f])= [1, tau_min, -1];
    g_lb(cstrInd) = -inf;
